...
namespace simplearchive {
	...
	bool CSVTable::updateList(std::shared_ptr<std::vector<std::shared_ptr<OptionItem>>> optionList)
	{
		MTTableSchema& schema = m_resultsList->getTableSchema();
		for (auto i = optionList->begin(); i < optionList->end(); i++) {
			std::shared_ptr<OptionItem> optionItem = *i;
			size_t idx = schema.getIndex(optionItem->getOption().c_str());
			...
			for (auto i = m_resultsList->begin(); i != m_resultsList->end(); i++) {
				...
				m_mtTablePartition->updateRow(m_indexRoot.c_str(), *row, idx, optionItem->getValue().c_str());
			}
		}
		return true;
	}
