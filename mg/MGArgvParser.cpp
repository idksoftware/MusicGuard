...
void  MGArgvParser::defineOptions() {
	...
	setIntroductoryDescription("iaarc - Image archive client provides the means to operate MusicGuard from the command line");
	...
	defineCommandSyntax("import", "iaarc import [--source-path=<path>][--scanfile=<path>]\n\t"
	...
	*/
	...
	defineOption("rs", "Remode server mode", ArgvParser::OptionAttributes::NoOptionAttribute);
	defineOptionAlternative("rs", "remote-server");
	...
	defineOption("T", "level", ArgvParser::OptionAttributes::NoOptionAttribute);
	defineOptionAlternative("T", "level");
	...
	defineOption("A", "current template (master or derivative)", ArgvParser::OptionAttributes::OptionRequiresValue);
	...
	defineCommandOption("import", "set");
	...
	defineCommandOption("export", "set");
	...
	defineCommandOption("template", "level");
...
bool MGArgvParser::doInitalise(int argc, char **argv) {
...
//	defineCommandOption("mode", "remote-server");
	...
	std::shared_ptr <About>		aboutCmd		= std::make_shared<About>(*this);
	std::shared_ptr <Import>		importCmd		= std::make_shared<Import>(*this);
	...
	std::shared_ptr <IaarcJournal>		journalCmd		= std::make_shared<IaarcJournal>(*this);
	...
	std::shared_ptr <IaarcScan>			iaarcScan		= std::make_shared<IaarcScan>(*this);
...
std::string MGArgvParser::usageDescriptionHeader(unsigned int _width) const
{
	std::string usage;
	
	//usage += "usage: imgarc subcommand [options] [args]\n\n";
	//usage += "MusicGuard command line client, version 1.0.0.1\n";
	//usage += "Type 'iaarc help <subcommand>' for help on a specific subcommand.\n\n";
	
	// setIntroductoryDescription("iaarc - Image archive client provides the means to operate MusicGuard from the command line");
	std::string tmp = "iaarc - Image archive client provides the means to operate MusicGuard from the command line";
	usage += '\n';
...
std::string MGArgvParser::commandUsage(unsigned int width) const
{
...
std::string MGArgvParser::generalHelp(unsigned int _width) const
{
	std::string usage; // the usage description text
	...
	*/
	std::string tmp = "The command imgarc is the primary command-line interface to MusicGuard. This interface is used to manage the control of images going in and out of the archive software. ";
