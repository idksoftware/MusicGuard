...
namespace simplearchive {
	...
	/*
	int ExifDateTime::operator==(ExifDateTime& d1)
	{
		if (m_year != d1.m_year) return false;
		if (m_month != d1.m_month) return false;
		if (m_day != d1.m_day) return false;
		if (m_hour != d1.m_hour) return false;
		if (m_min != d1.m_min) return false;
		if (m_sec != d1.m_sec) return false;
		return true;
	}
	*/
