...
namespace simplearchive {
	...
	const char* MTSchema::getTypeString() {
		...
		case MTType::UInteger: return "Uinteger";
	...
	bool MTColumn::operator==(const MTColumn& r)
	{
		MTType type = m_info->getType();
		if (r.m_object == nullptr) {
			m_object = nullptr;
		...
		switch (type) {
		case MTType::Integer:
			return (m_object->getInt() == (r.m_object->getInt()));
		case MTType::UInteger:
			return (m_object->getUInt() == (r.m_object->getUInt()));
		case MTType::Text:
			return (m_object->getString() == (r.m_object->getString()));
		case MTType::Date:
		{
			ExifDateTime d1 = m_object->getDate();
			ExifDateTime d2 = r.m_object->getDate();
			return (d1 == d2);
		}
		case  MTType::Float:
			return (m_object->getDouble() == (r.m_object->getDouble()));
			break;
		}
		return false;
	}
	...
	*/
	...
	MTRow::MTRow(MTTableSchema& schemaTable) : m_delim(':')
	{
		m_schema = std::make_shared<MTTableSchema>(schemaTable);
	...
	MTRow::MTRow(std::shared_ptr<MTTableSchema> schema) : m_delim(':')
	{
		m_schema = schema;
		for (auto i = m_schema->begin(); i != m_schema->end(); i++) {
			std::shared_ptr<MTColumn> col = std::make_shared<MTColumn>(*i);
	...
	MTRow::MTRow(MTTableSchema& schemaTable, char delim) : m_delim(delim)
	{
		m_schema = std::make_shared<MTTableSchema>(schemaTable);
		for (auto i = m_schema->begin(); i != m_schema->end(); i++) {
			std::shared_ptr<MTColumn> col = std::make_shared<MTColumn>(*i);
	...
	MTRow::MTRow(const MTRow& row) : m_schema(row.m_schema), m_delim(row.m_delim) {
		m_schema = std::make_shared<MTTableSchema>(*(row.m_schema));
		for (auto i = m_schema->begin(); i != m_schema->end(); i++) {
			std::shared_ptr<MTColumn> col = std::make_shared<MTColumn>(*i);
			this->emplace_back(col);
		}
	...
	*/
	...
	SharedMTRow MTTable::makeRow() {
		auto row = std::make_shared<MTRow>(m_TableSchema);
		return row;
	}
	...
	bool MTTable::fromString(const std::string& r) {
		auto rowPtr = std::make_shared<MTRow>(m_TableSchema);
	...
	void MTTableSchema::make(MTRow& row) {
