...
namespace simplearchive {
	...
	public:
		...
		virtual ~MTTypeException() noexcept {}
	...
	public:
		...
		bool operator==(const MTColumn& r);
	...
	class MTRow : public std::vector<SharedMTColumn> {
		std::shared_ptr<MTTableSchema> m_schema;
		//std::string m_text;
		char m_delim;
	protected:
	...
	public:
		...
		MTRow(std::shared_ptr<MTTableSchema> schema);
		...
		*/
		void debugPrint() {
			std::cout << "\n";
			std::cout << m_schema->getName() << "\n";
			std::cout << "===================" << "\n";
			int r = 0;
			for (auto i = m_schema->begin(); i != m_schema->end(); i++) {

				MTSchema column = *i;
				MTColumn& col = columnAt(r++);
				
				std::cout << column.getName() << " Type: " << column.getTypeString() << " : " << col.toString() << "\n";
			}
		}
		...
		void setSchema(MTTableSchema& schema) {
			m_schema = std::make_shared<MTTableSchema>(schema);
		}
	};
