...
*
*	The Python Image Library is a python library that can be used to extract EXIF MusicGuard product provides a python
...
MetadataObject *ExternalComand::process(const char *path, const char* file) {
	...
	if (findToken(m_commandLine, "exif") == false) {
		...
		logger.log(LOG_OK, CLogger::Level::STATUS, "Using external Exif command: \"%s\"", cmd.c_str());
		if (externalShell.exec(cmd.c_str()) == false) {
			logger.log(LOG_OK, CLogger::Level::WARNING, "External Exif command failed \"%s\"", cmd.c_str());
			return nullptr;
		}
		...
		metadataObject = externalExifMapper.create(externalShell.getOutput());
	}
	else {
		...
		metadataObject = externalExifMapper.create(out.c_str());
	}
